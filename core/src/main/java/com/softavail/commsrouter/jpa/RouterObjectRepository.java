...
public class RouterObjectRepository<ENTITYT extends RouterObject>
  ...
  public ENTITYT get(EntityManager em, RouterObject routerObject) throws NotFoundException {
    ENTITYT entity = get(em, routerObject.getId());
    if (entity != null && Objects.equals(entity.getRouterId(), routerObject.getRouterId())) {
      return entity;
    }
    throw new NotFoundException(entityClass.getSimpleName() + ": " + routerObject + " not found");
  }
  ...
  public void delete(RouterObject routerObject) throws CommsRouterException {
    transactionManager.executeVoid((em) -> {
      ENTITYT entity = em.find(entityClass, routerObject.getId());
      if (entity != null && Objects.equals(entity.getRouterId(), routerObject.getRouterId())) {
        em.remove(entity);
      }
    });
