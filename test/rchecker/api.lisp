...
(defun router-all()
  (tr-step (http-get "/routers")
           #'(lambda(js)(and (listp js) (> (length js) 0) (funcall (contains "id") (first js))))
           #'(lambda(js) (funcall (fire-event :router) (jsown:val (first js) "id")))))
...
(defun router-new(&key (name "name") (description "description"))
  (tr-step (http-post "/routers" (jsown:new-js("name" name)
           ...
           #'(lambda(js) (funcall (fire-event :router) (jsown:val js "id")))))
...
(defun plan-all(&key (router-id (get-event :router)) )
  (tr-step (http-get "/routers" router-id "plans")
           #'(lambda(js) (and (listp js) (funcall (contains "id") (first js))))
           #'(lambda(js) (funcall (fire-event :plan) (jsown:val (first js) "id")))))
...
(defun task-all(&key (router-id (get-event :router)) )
  (tr-step (http-get "/routers" router-id "tasks")
           #'(lambda(js) (and (listp js) (funcall (contains "id") (first js))))
           #'(lambda(js) (funcall (fire-event :task) (jsown:val (first js) "id")))))
